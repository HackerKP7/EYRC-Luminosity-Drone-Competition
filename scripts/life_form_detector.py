...
class swift ():
	...
	def __init__(self):
		...
		# self.finish = False
		# self.reverse = False
		self.setpoint = [[-8.5,-8.5,25],[-3.5,-8.5,25],[3.5,-8.5,25],[8.5,-8.5,25],[8.5,-3.5,25],[3.5,-3.5,25],[-3.5,-3.5,25],[-8.5,-3.5,25],[-8.5,3.5,25],[-3.5,3.5,25],[3.5,3.5,25],[8.5,3.5,25],[9,9,25] ,[3.5,9,25] ,[-3.5,9,25],[-9,9,25],[0,0,25],[10.5,10.5,30]] 
		self.l = 0
		self.alien = "zero"
	...
	def pid(self):
		if self.l == 18 :
			...
			#------------------------------------------------------------------------------------------------------------------------
				...
				if -0.2 < self.error[0] < 0.2 and -0.2 < self.error[1] < 0.2 and -0.2 < self.error[2] < 0.2 and self.l < 18:
							print("reached next ",self.l,"location:",self.drone_position)
							
							if self.l == 16:
								cv.imwrite('bruh.jpg',self.led)
							...
							if len(contours) > 0 and self.l < 16:
										...
										if self.drone_position[0] < 0:
											self.setpoint[16][0] = (sx/2.6) + self.drone_position[0]
										if self.drone_position[1] < 0:
											self.setpoint[16][1] = (sy/2.6) + self.drone_position[1]
										if self.drone_position[0] > 0:
											self.setpoint[16][0] =  self.drone_position[0] - (sx*2.6)
										if self.drone_position[1] > 0:
											self.setpoint[16][1] =  self.drone_position[1] - (sy*2.6)
										
										print('organisms coor: ',sx,sy)
										
								

										if len(contours) == 2:
											self.alien = "alien_a"
											print("Alien_A detected")

										elif len(contours) == 3:
											self.alien = "alien_b"
											print("Alien_B detected")

										elif len(contours) == 4:
											self.alien = "alien_c"
											print("Alien_C detected")
										self.astrobiolocation.publish(self.alien,self.setpoint[16][0],self.setpoint[16][1],37)
										self.l = 16
										cv.waitKey(0)
