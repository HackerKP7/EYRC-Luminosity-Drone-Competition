...
class swift ():
	...
	#	8. Add error_sum
			for i in range(0,len(self.error)):
				...
				if i == 2:
					self.cmd.rcThrottle = int((1570 + (self.error[i] * self.Kp[i])) + ((self.error[i] - self.prev_error[i]) * self.Kd[i]) + (self.sum_error[i] * self.Ki[i]))
	...
	#------------------------------------------------------------------------------------------------------------------------
			...
			if -0.2 < self.error[0] < 0.2 and -0.2 < self.error[1] < 0.2 and -0.2 < self.error[2] < 0.2 :
								self.l = self.l + 1
								print("reached next ",self.l,"location:",self.drone_position)
			else:
				print("out    ",self.drone_position)
